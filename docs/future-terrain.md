...
- **Near-plane tuning.** Derive the point-shadow near plane from eye/source height so precision improves without clipping the immediate terrain. Double-sided shadow casting produces better visual results for now, but future work needs interpretable shadow-depth buffers which would be compromised by this.
