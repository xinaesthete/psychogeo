...
| Lazy per-tile raster channels (height variants, aux, photo) | `TerrainLayer` expects standard tile schema; attaching aux rasters per-tile is off-grid | Imagery providers plug into globe tiling, not arbitrary channels | Today's compression experiment proves the pattern; needs generalising (see [tile-layers.md](tile-layers.md)) | Same pattern; compute passes could decode in-place |
